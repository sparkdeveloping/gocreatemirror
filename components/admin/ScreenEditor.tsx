...
function WidgetInspector({ widget, updateConfig, uploadPhoto }: { widget: ScreenWidget; updateConfig: (patch: Partial<ScreenWidget["config"]>) => void; uploadPhoto: (event: ChangeEvent<HTMLInputElement>) => Promise<void> }) {
  ...
  if (["teamNow", "teamNext", "teamToday", "teamWeek"].includes(widget.type)) return <section><h3>Team schedule</h3>{field("Title", "scheduleTitle")}<label className="inspector-field wide"><span>Maximum people</span><input type="number" min="1" max="30" value={widget.config.scheduleMaxItems || (widget.type === "teamNow" ? 8 : widget.type === "teamNext" ? 6 : 20)} onChange={(event) => updateConfig({ scheduleMaxItems: Number(event.target.value) })}/></label><Toggle label="Show shift times" checked={widget.config.scheduleShowTimes !== false} onChange={(scheduleShowTimes) => updateConfig({ scheduleShowTimes })}/><Toggle label="Show opening / mid / closing labels" checked={widget.config.scheduleShowLabels !== false} onChange={(scheduleShowLabels) => updateConfig({ scheduleShowLabels })}/><small className="inspector-note">Roster data comes from Admin → Team Schedule. Position, scale, rotation, colors and animation remain fully editable here.</small></section>;
